...
public class EchoTestServiceTests {
    ...
    public void futureCausesOperationError(final CompletableFuture<?> future, Class<? extends EventStreamOperationError> clazz, String code) {
        try {
            ...
            if (t == null) {
                Assertions.fail("ExecutionException thrown has no caused by exception. Something else went wrong with future completion");
    ...
    public void testInvokeEchoStreamError() throws Exception {
        final CompletableFuture<Void> clientErrorAfter = EchoTestServiceRunner.runLocalEchoTestServer((connection, client) -> {
            ...
            final CauseStreamServiceToErrorResponseHandler responseHandler = client.causeStreamServiceToError(EchoStreamingRequest.VOID, Optional.of(new StreamResponseHandler<EchoStreamingMessage>() {
                @Override
                public void onStreamEvent(EchoStreamingMessage streamEvent) {
            ...
            try {
                ...
                responseHandler.sendStreamEvent(msg);   //sends message, exception should be is the response
