...
if git diff --exit-code VERSION > /dev/null; then
    ...
    python3 update-crt.py ${version_without_v} --update_sdk_text
    python3 update-crt.py ${version_without_v} --update_samples
    ...
    # push the commit
    git push -u "https://${GITHUB_ACTOR}:${GITHUB_TOKEN}@github.com/aws/aws-iot-device-sdk-java-v2.git" ${version_branch}
    ...
    # delete the old tag
    git tag -d ${version}
    git push "https://${GITHUB_ACTOR}:${GITHUB_TOKEN}@github.com/aws/aws-iot-device-sdk-java-v2.git" :refs/tags/${version}
